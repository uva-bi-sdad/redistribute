...
block_groups$income_lt50 <- rowSums(
  block_groups[, grep("_(?:lt|[1-5]{2}_).*\\d$", income_vars)]
)
block_groups$income_100_200 <- rowSums(
  block_groups[, grep("_1\\d{2}.*\\d$", income_vars)]
)
income_vars_select <- c("income_lt50", "income_100_200", "income_gt200")
...
parcels$Residents <- vapply(seq_len(nrow(parcels)), function(i) {
  parcels$Total[[i]] * block_groups$avg_hhsize[
    block_groups$GEOID == parcels$geoid[[i]]
  ]
}, 0)
...
pums$household$income_cat <- as.character(cut(
  pums$household$HINCP, c(-Inf, 50, 200, Inf) * 1e3, income_vars_select
))
...
rake_prep <- function() {
  eval(expression({
    for (var in names(totals)) {
      ...
      if (length(su)) totals[[var]] <- totals[[var]][!su]
...
est_grake <- function(source, target, individuals) {
  ...
  individuals$weight <- tryCatch(
    suppressWarnings(ml_fit_hipf(m)$weights),
    error = function(e) {
      warning(e$message)
      NULL
    }
...
```{r, echo=FALSE, fig.height=4, fig.width=8.1}
library(leaflet)
...
library(splot)
for (var in c("race_white", "race_native", "income_100_200", "Average")) {
