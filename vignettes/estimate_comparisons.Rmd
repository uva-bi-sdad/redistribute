...
parcels$Residents <- vapply(seq_len(nrow(parcels)), function(i) {
  parcels$Total[[i]] * block_groups$avg_hhsize[
    block_groups$GEOID == substring(parcels$Full_Block[[i]], 1, 12)
  ]
}, 0)
...
pums$household$income_cat <- as.character(cut(pums$household$HINCP, c(
  ...
  200, Inf
) * 1e3, c("income_lt10", colnames(tracts)[income_cols], "income_gt200")))
...
apply_method <- function(data, method) {
  p <- do.call(rbind, lapply(data, function(set) {
    ...
    do.call(rbind, lapply(unique(set$source$id), function(id) {
      ...
      if (nrow(target)) {
        # adjust resident totals to match source totals
        target$Renter_Occupied <- target$Renter_Occupied * source$avg_hhsize_rent
        target$Owner_Occupied <- target$Owner_Occupied * source$avg_hhsize_own
        vcols <- c("Renter_Occupied", "Owner_Occupied")
        ptotals <- rowSums(target[, vcols])
        target[, vcols] <- target[, vcols] / ptotals *
          (sum(source[, 1:2]) * ptotals / sum(target[, vcols]))
...
write.csv(error, paste0(base_dir, "/results.csv"), row.names = FALSE)
```
...
if (file.exists(results_altgeo_file)) {
  ...
  results_altgeo <- lapply(1:10, apply_altgeo)
