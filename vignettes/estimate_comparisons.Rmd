...
parcels$geoid <- substring(parcels$Full_Block, 1, 12)
block_groups <- block_groups[block_groups$GEOID %in% parcels$geoid, ]

## from tracts to parcels
map_tr_to_parcel <- split(
  unlist(unname(map_bg_to_parcel)),
  rep(substring(names(map_bg_to_parcel), 1, 11), vapply(map_bg_to_parcel, length, 0)),
)
...
we may be able to refine our redistribution using the average household
size estimated in the ACS block group summaries:

```{r}
# multiply the number of living units in each parcel
# by the average household size estimate in the parent block group
parcels$Residents <- parcels$Total * structure(
  block_groups$avg_hhsize,
...
vars <- c(
  unlist(vars_list, use.names = FALSE),
  "avg_hhsize_own", "avg_hhsize_rent"
)
...
est_grake <- function(source, target, individuals) {
  ...
  observed_types <- tapply(rowSums(target[, vars_units[-1]]), target$Unit_Type, sum)
  ...
  individuals$weight <- if (is.null(fm$weight)) individuals$PWGTP else fm$weight
  environment(make_estimates) <- environment()
  make_estimates()
}
...
```{r, echo=FALSE}
# save for site
error <- do.call(cbind, lapply(structure(vars, names = vars), function(var) {
  d <- cbind(
...
So far, we've used the same data at different initial geolayers
...
if (file.exists(population_file)) {
  ...
  population <- generate_population(sum(parcels$Total), parcels, "Total", "OBJECTID")
  ...
  population$individuals$race_cat <- rep_len(
    grep("^race", vars, value = TRUE), max(population$individuals$race) + 1
...
draw_sample <- function(run = NULL, return_full = FALSE) {
  ...
  bg <- do.call(rbind, lapply(split(individuals, individuals$geoid), function(d) {
    ...
    cbind(
      ...
      as.data.frame(unlist(lapply(
        names(vars_list), function(var) {
          as.list(vapply(
            unique(vars_list[[var]]), function(l) sum((d[[var]] == l) * d$PWGTP), 0
          ))
        }
      ), recursive = FALSE)),
  ...
  ps$Residents <- ps$Total * structure(bg$avg_hhsize, names = bg$id)[ps$geoid]

  vdata <- lapply(structure(pumas_focal, names = pumas_focal), function(puma) {
    ps <- ps[ps$id %in% data[[puma]]$parcels$OBJECTID, ]
...
We can also look at results across samples from the same simulated population:

```{r, warning=FALSE}
results_sim_file <- paste0(base_dir, "/results_sim.rds")
if (file.exists(results_sim_file)) {
  results_sim <- readRDS(results_sim_file)
} else {
  results_sim <- lapply(1:50, draw_sample)
  saveRDS(results_sim, results_sim_file)
}

kable(Reduce("+", results_sim) / length(results_sim), digits = 5)
```

For some reason, only the estimation methods vary at all in this case:

```{r, echo=FALSE, fig.height=4.1, fig.width=8.1}
library(splot)
for (var in c("sex_female", "race_black", "income_gt200", "Average")) {
  dist <- do.call(rbind, lapply(results_sim, function(r) r[var, -(1:2)]))
  splot(
    dist,
    title = var, labx = "Mean Absolute Error", laby = "Density",
    leg.title = "Methods", leg = "out"
