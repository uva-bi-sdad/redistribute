...
## for parcels with more than 1 unit, assume 2.5 per unit
su <- parcels$Total > 1
parcels$Residents[su] <- parcels$Total[su] * 2.5

## for 1-unit parcels, bases on size
su <- parcels$Total == 1
parcels$Residents[su] <- as.numeric(cut(
  parcels$Shape__Area[su], c(-Inf, 6000, 9000, 20000, 40000, Inf)
)) + 4
...
if (file.exists(population_file)) {
  ...
  population <- generate_population(
    sum(parcels$Total), parcels, "Total", "Shape__Area", "OBJECTID"
  )
...
draw_sample <- function(run = NULL, return_full = FALSE) {
  ...
  ps$Residents <- parcels$Total
  su <- ps$Total > 1
  ps$Residents[su] <- ps$Total[su] * 2.5
  
  ps$Shape__Area <- structure(
    parcels$Shape__Area,
    names = parcels$OBJECTID
  )[ps$id]
  su <- ps$Total == 1
  ps$Residents[su] <- as.numeric(cut(
    ps$Shape__Area[su], c(-Inf, 6000, 9000, 20000, 40000, Inf)
  )) + 4
