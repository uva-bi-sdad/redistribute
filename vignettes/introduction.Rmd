...
region_values <- redistribute(
  set_value, regions, weight = c(1, 10, 10, 20, 50), verbose = TRUE
)
region_values
```
...
population[, -1] <- vapply(population[, -1], as.numeric, numeric(nrow(population)))
...
map <- leaflet(population, options = leafletOptions(attributionControl = FALSE)) |>
  ...
  addLayersControl(
    position = "topleft", overlayGroups = c(
      "Block Groups", "Block Group -> Zip Codes", "Parcels -> Zip Codes"
    )
  ) |>
  addLegend(
  ...
  addPolygons(
    ...
    group = "Block Groups", label = ~ paste(GEOID, "Population:", round(Overall, 3))
  )
```
...
if (!file.exists(zipcode_file)) {
  download.file(paste0(
    "https://www.fairfaxcounty.gov/euclid/rest/services/IPLS/IPLSMap/MapServer/3/query?",
    'geometry={"xmin":-90,"ymin":-90,"xmax":90,"ymax":90,"spatialReference":{"wkid":4326}}',
    "&outFields=*&f=geojson"
  ), zipcode_file)
}
...
zipcode_population <- redistribute(
  population, zipcodes, target_id = "ZIPCODE", verbose = TRUE
)
```
...
if (!file.exists(parcel_file)) {
  download.file(paste0(
    "https://opendata.arcgis.com/api/v3/datasets/314bfe4019754952a715be3a33384d9d_0/",
...
# redistribute population data from block groups
parcel_population <- redistribute(population, parcels, weight = "CURRE_POPUL", verbose = TRUE)
parcel_zipcode_population <- redistribute(
  parcel_population, zipcodes, source_id = "id", target_id = "ZIPCODE", verbose = TRUE
)
```

```{r}
pal_bg <- colorNumeric(scico::scico(255, palette = "lajolla"), zipcode_population$Overall)
pal_pa <- colorNumeric(scico::scico(255, palette = "lajolla"), parcel_zipcode_population$Overall)
map |>
  addPolygons(
    data = zipcode_population,
    fillColor = ~ pal_zip(Overall), fillOpacity = .8, weight = 1,
    color = "#000", highlightOptions = highlightOptions(color = "#fff"),
    group = "Block Group -> Zip Codes", label = ~ paste(
      id, "Population:", round(Overall, 3)
    )
  ) |>
  addLegend(
    "bottomright", pal_zip, zipcode_population$Overall,
    opacity = 1, title = "Block Group -> Zip Codes", group = "Block Group -> Zip Codes"
  ) |>
  addPolygons(
    data = zipcode_population,
    fillColor = ~ pal_zip(Overall), fillOpacity = .8, weight = 1,
    color = "#000", highlightOptions = highlightOptions(color = "#fff"),
    group = "Parcels -> Zip Codes", label = ~ paste(id, "Population:", round(Overall, 3))
  ) |>
  addLegend(
    "bottomright", pal_zip, zipcode_population$Overall,
    opacity = 1, title = "Parcels -> Zip Codes", group = "Parcels -> Zip Codes"
  )
```
