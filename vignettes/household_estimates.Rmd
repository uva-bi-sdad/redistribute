...
Note that some block groups contain no parcels despite having
non-zero populations, so our parcel-based estimates will be off total
(there are `r sum(block_groups$Total) - sum(estimates_parcels$Total)` people unaccounted for by parcels):
```{r}
block_groups[
  block_groups$GEOID %in% names(which(vapply(map_bg_to_parcel, length, 0) == 0)),
  c("GEOID", "Total"),
  drop = TRUE
]
```
...
Despite the under-coverage of block groups by parcels, the estimated number of households
at the parcel level is slightly higher (which could also be influenced by the difference in time):
```{r}
totals <- c(
...
As in the direct case, we can also see how much of an improvement weighting by
household estimates might make:
```{r}
# recalculate the parcel estimates
estimates_parcels_hh <- redistribute(
  block_groups, parcels_hh, map_bg_to_parcel,
  target_id = "PIN", weight = "CURRE_HHLDS"
)

mean(abs(estimates_parcels_hh$Total - parcels_hh$CURRE_HHLDS), na.rm = TRUE)
cor(estimates_parcels_hh$Total, parcels_hh$CURRE_HHLDS, use = "complete.obs")

# reaggregate to zipcodes
estimates_downup_hh <- redistribute(
  estimates_parcels_hh, zipcodes, map_parcel_to_zip,
  source_id = "id", target_id = "ZIPCODE", default_value = 0
)

mean(abs(estimates_downup_hh$Total - zipcodes$HOUSEHOLDS))
cor(estimates_downup_hh$Total, zipcodes$HOUSEHOLDS)
```

The parcel-level household estimates differ very little from the unit counts,
so it makes less of a difference in this case:
```{r}
...
data <- cbind(
  ...
  DownUpHH = estimates_downup_hh$race_white,
...
leaflet(
  ...
  addLayersControl(
    position = "topleft",
    baseGroups = c(
      "Direct", "Direct Weighted", "DownUp", "DownUp Households", "Pums", "Rake"
    )
  ) |>
  ...
  addPolygons(
    fillColor = ~ pal(DownUpHH), fillOpacity = 1, weight = 1,
    color = "#000", highlightOptions = highlightOptions(color = "#fff"),
    group = "DownUp Households", label = ~ paste(ZIPCODE, "White:", DownUpHH)
  ) |>
  hideGroup("DownUp") |>
  addPolygons(
...
data_native <- cbind(
  ...
  DownUpHH = estimates_downup_hh$race_native,
...
leaflet(
  ...
  addPolygons(
    fillColor = ~ pal(DownUpHH), fillOpacity = 1, weight = 1,
    color = "#000", highlightOptions = highlightOptions(color = "#fff"),
    group = "DownUp Households", label = ~ paste(ZIPCODE, "Native:", DownUpHH)
  ) |>
  hideGroup("DownUp") |>
  addPolygons(
    fillColor = ~ pal(Pums), fillOpacity = 1, weight = 1,
